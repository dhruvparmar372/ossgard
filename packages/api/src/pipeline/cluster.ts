...
/** Options for the standalone runCluster function. */
export interface RunClusterOpts {
  prs: PR[];
  scanId: number;
  repoId: number;
  accountId: number;
...
/**
 * Standalone cluster logic: resolve vector store, run UnionFind clustering, search.
 * Does NOT update scan status or enqueue jobs — the caller handles that.
 * Returns candidate groups for the verify phase.
 */
export async function runCluster(opts: RunClusterOpts): Promise<CandidateGroup[]> {
